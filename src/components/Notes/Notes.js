...
class Notes extends React.Component {
    ...
    render() {
        return (
            <div>
                {this.props.noteState.notesAllLoading ? null : (
                    <div className={`input-group mb-3`}>
                        <div className={`input-group-prepend`}>
                            <span className={`input-group-text`} id={`searchField`}>
                                Search
                            </span>
                        </div>
                        <input className={`${css.searchField} form-control`} placeholder={`query...`} type={`text`} onChange={(e) => this.props.search(e.target.value)} value={this.props.searchValue} aria-describedby={`searchField`} />
                    </div>
                )}
                {this.props.noteState.notesAllLoading ? (
                    <div className={css.notesLoader}></div>
                ...
                {this.props.noteState.notesAllLoading ? null : (
                    <button className={`btn btn-primary`} onClick={() => this.props.noteToggleAllCompleted(!this.props.noteState.showCompleted)}>
                        {this.props.noteState.showCompleted ? `Hide` : `Show`} completed
                    </button>
                )}
                {
                    <CSSTransition
                        ...
                        classNames={{
                            enter: css.noteModalEnter,
                            enterActive: css.noteModalActiveEnter,
                        }}
                        unmountOnExit
                        mountOnEnter
                    >
